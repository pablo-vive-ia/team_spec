...
- **Scope toggle** (`activeTicketScope`, default `'abiertos'`, reset en `switchSection`): pills "Abierto · Nuevo" / "Cerrados" / "Todos" sobre la tabla principal — reemplazan el badge estático que había antes. `ticketMatchesScope(t, scope)` es el único punto que decide inclusión por estado; `isTicketAbierto(t)` (basado en `STATUS_ABIERTOS = ['abierto','nuevo','open','new','pendiente']`) es la fuente de verdad de qué es "abierto" en toda la sección (KPIs, reconciliación, filtro). `'pendiente'` se agregó el 2026-09-17 para que coincida con la lista que ya usaba la reconciliación de huérfanos del workflow n8n (`Comparar y Preparar`) — antes el frontend no la tenía, así que un ticket en un estado "Pendiente..." de Zammad hubiera desaparecido de Abiertos en el dashboard aunque siguiera abierto en Zammad (no llegó a pasar en la práctica: ningún ticket tuvo ese estado literal hasta ahora, pero era una inconsistencia latente).
...
- **Grid responsivo**: `repeat(auto-fit,minmax(215px,1fr))` (no `grid-auto-flow:column`) — las columnas se achican para entrar todas en una fila a medida que crece el equipo, en vez de forzar scroll horizontal apenas se pasan de ~4-5 personas. `overflow-x:auto` queda como red de seguridad en viewports angostos. Pase de compactación (2026-09-03): tarjetas, header de columna y chips de stats con menos padding/font-size que la versión inicial — pedido explícito del usuario ("que ocupe menos pantalla").
- **Fecha visible en formato `dd/mm/aaaa`** vía `dmy(s)` — mismo criterio que `fmtDate()` para el resto de la app (nunca relativo). Reemplazó a `shortDate()` (día de semana + día/mes, sin año, tono apagado `--t3`), que se eliminó por no usarse más. En la tarjeta va en `--t1`/11px/800 (antes `--t3`/9.5px) para que se note a simple vista; en la vista Tabla, `--t1`/mono/700.
...
- **Filtro de responsable = una sola columna** (solo en agrupamiento por miembro): con `activeTaskAssignee` distinto de `'Todos'`, `renderTareasTableroPorMiembro(b, order, filterAssignee)` muestra únicamente la columna de esa persona — el resto desaparece (no se listan vacías). El dataset ya viene pre-filtrado desde `renderTareas()`, así que alcanza con restringir `visible` a `[filterAssignee]`. El filtro de responsable sigue aplicando también cuando se agrupa por prioridad (recorta tarjetas dentro de cada columna), solo que ahí no colapsa columnas.
- **Agrupamiento del tablero: por miembro o por prioridad** (2026-09-04, pedido explícito: "que pueda organizarlo por Prioridad también, no solo por miembro"). `taskGroupBy` (`'assignee'` default | `'priority'`, persistido en `localStorage['spec-task-group']`, **no** se resetea en `switchSection` — mismo criterio que `taskView`) decide entre `renderTareasTableroPorMiembro()` (columnas = personas, la que ya existía) y `renderTareasTableroPorPrioridad()` (columnas fijas `urgente → alta → media → baja` vía `TASK_PRIO_KEYS`, siempre las 4 visibles aunque estén vacías). El toggle de pills solo se muestra con `taskView==='tablero'` (no aplica a la vista Tabla). El drag & drop también funciona agrupado por prioridad: soltar una tarjeta sobre una columna le cambia la prioridad (`dropTaskOnPriority` → `reprioritizeTask`, mismo patrón que `dropTaskOnAssignee`/`reassignTask`). `renderTareasTablero(b, order, filterAssignee, groupBy)` es ahora solo un despachador entre las dos.
- **Reasignar responsable, dos vías** (solo admin, agrupamiento por miembro): **drag & drop** (cada tarjeta es `draggable`, `dataTransfer` lleva el id; el body de cada columna es drop zone vía `dropTaskOnAssignee(event, name)`, con feedback visual al pasar por encima) o **editando la tarea** desde el modal (campo RESPONSABLE). Ambas llaman/pasan por `canonAssignee()` antes de guardar. Hubo una tercera vía (`<select>` inline en la tarjeta, `reassignTask(id, rawAssignee)` llamado directo desde ahí) que se sacó el 2026-09-03 a pedido del usuario — quedaba visualmente redundante con el nombre de columna. `reassignTask()` sigue existiendo (lo usa `dropTaskOnAssignee`).
...
- **Color por estado** (2026-09-04, pedido explícito): `TASK_STATUS_BG`/`TASK_STATUS_FG` — `pendiente` amarillo (`--warn`), `en_progreso` azul (`--acc`, ya lo era), `completado` verde (`--success`, ya lo era), `cancelado` rojo (`--danger`, antes gris apagado). Un solo punto de verdad para el chip de estado en tarjeta y en la vista Tabla (`statusChipHtml()`).
- **Historial de cambios de estado** (2026-09-03, pedido explícito: "por si cometo un error"): reusa `status_log` (antes solo `project`/`ticket`/`order`/`installation`, escrito por n8n con `service_role`) — se agregó `'task'` al enum `entity_type` y una policy `admin insert status_log` (`TO authenticated WITH CHECK (public.is_admin())`) para que el frontend también pueda escribir ahí. `logTaskStatusChange(id, prevStatus, newStatus)` es fire-and-forget (un fallo solo se loguea en consola, no bloquea la acción principal) y se llama desde `submitTask()`, `cycleTaskStatus()` y `toggleTaskDone()` — **solo cambios de estado, no reasignaciones** (alcance literal del pedido). `created_by` guarda `myProfile.full_name || myProfile.email`, ahora que hay sesión real. Se ve en el modal de edición: `loadTaskHistory(id)` trae las últimas 15 entradas de `status_log` para esa tarea y las pinta en `#task-history` (oculto si no hay historial o si es una tarea nueva).
...
- `product` en Zammad llega como array JSON (`["x-Time"]`) — se extrae tomando el **último** elemento del array (`Array.isArray(rawProducto) ? rawProducto[rawProducto.length-1] : rawProducto`).
  - **Fix 2026-09-17** (antes se tomaba `rawProducto[0]`, el primero): el campo "Producto" de Zammad es en el fondo multi-selección, y un ticket puntual (`#202609013500053`) tenía **dos valores cargados a la vez** (`x-Time::Reparacion Hardware Laboratorio` y `SPECManager::Reparacion Hardware Laboratorio`). Tomar el primero lo dejaba pegado en `x-Time` para siempre — como el array llega idéntico en cada corrida, el diff de `Comparar y Preparar` nunca detectaba cambio, así que **ni resincronizar a mano lo corregía**. El último elemento coincide con lo que Zammad muestra como valor vigente en la UI. Motivo de que apareciera: causaba que el conteo de "Abiertos" de SPECManager diera 1 de menos y el de xTime 1 de más — detectado porque el usuario comparó a mano contra los badges del sidebar de Zammad. Verificado sobre los 113 tickets abiertos al momento del fix: era el único caso con producto multi-valor.
